...
// SQLファイルの読み込み
$schema_sql = file_exists("database/schema.sql") ? file_get_contents("database/schema.sql") : "-- Schema file not found";
if (!file_exists("database/schema.sql")) {
    $message = "❌ SQLファイルが存在しません。";
    $status = "error";
}

$insert_sql = file_exists("database/insert_data.sql") ? file_get_contents("database/insert_data.sql") : "-- Insert data file not found";
if (!file_exists("database/insert_data.sql")) {
    $message = "❌ SQLファイルが存在しません。";
    $status = "error";
}
