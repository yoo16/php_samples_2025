...
function get($limit = 50)
{
    try {
        ...
        // SQL作成：users テーブルから指定件数取得
        $sql = "SELECT * FROM users LIMIT :limit;";
        // プリペアドステートメントを使用して安全に実行
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':limit', (int)$limit, PDO::PARAM_INT);
        $stmt->execute();
        // Userデータを全件取得: fetchAll(PDO::FETCH_ASSOC)
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        // エラーハンドリング（本番環境ではログ出力が望ましい）
        return [];
    }
}
...
$title = 'ユーザ一覧取得 (READ)';
$lesson_number = 10;
$description = 'データベースから情報を取得して画面に表示する「READ（読み取り）」の基本です。SELECT文の発行と、fetchAllによるデータ受け取りの流れをマスターしましょう。';
?>
...
<body class="bg-slate-50 text-slate-800 leading-relaxed antialiased">
    ...
    <nav class="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div class="max-w-5xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <a href="index.php" class="text-sm font-semibold text-sky-600 hover:text-sky-700 transition">&larr; CRUDメニュー</a>
        </div>
    </nav>
                        ...
                        <?php else: ?>
                            <?php foreach ($users as $user): ?>
                                <tr class="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="font-bold text-slate-700">@<?= htmlspecialchars($user['account_name']) ?></span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <a href="delete_user.php?id=<?= $user['id'] ?>" class="p-1 text-slate-400 hover:text-rose-600 transition" title="削除">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
